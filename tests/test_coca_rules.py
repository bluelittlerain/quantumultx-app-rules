...
class COCAScopeTests(unittest.TestCase):
    ...
    def test_updater_uses_current_public_official_sources(self) -> None:
        ...
        self.assertEqual(len(updater.OPTIONAL_SOURCE_URLS), 1)
        self.assertEqual(
            updater.CONFIG.upstream_urls,
            updater.CORE_SOURCE_URLS + updater.OPTIONAL_SOURCE_URLS,
        )
        for redundant in ("terms", "privacy", "cards", "blog"):
            self.assertFalse(
                any(redundant in url for url in updater.CONFIG.upstream_urls)
            )
    ...
    def _assert_single_source_failure_is_degraded(
        ...
        self.assertEqual(len(sources.successful), 3)
        ...
        main = next(
            item for item in plan.files if item.path.name == "COCA.list"
        )
    ...
    def test_all_sources_failing_blocks_update_and_preserves_list(self) -> None:
        path = ROOT / "rule/QuantumultX/COCA/COCA.list"
    ...
    def test_too_few_observations_blocks_update(self) -> None:
        with self.assertRaisesRegex(utils.SafetyError, "observation count 1"):
            updater.prepare_resilient_update(
                fetcher=source_fetcher(payload="coca.xyz\n")
            )
    ...
    def test_partial_failure_keeps_manual_rules(self) -> None:
        ...
        self.assertIn("HOST,wwallet.app.link,COCA", main.new_content)
        self.assertIn("HOST-SUFFIX,coca.xyz,COCA", main.new_content)
        self.assertEqual(main.removed, 0)
